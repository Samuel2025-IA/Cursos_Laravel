/**
 * Sistema de Alertas de Bienvenida para Panel de Administración
 * Maneja alertas flash y mensajes de bienvenida específicos para administradores
 */

// Variable para controlar que solo se ejecute una vez
let adminWelcomeInitialized = false;

// Función para mostrar alertas flash
function showAdminFlashAlert() {
    const flashMessage = document.querySelector('meta[name="flash-message"]');
    const flashToken = document.querySelector('meta[name="flash-token"]');
    
    if (flashMessage && flashToken) {
        const message = flashMessage.getAttribute('content');
        const token = flashToken.getAttribute('content');
        
        // Verificar localStorage
        const processedTokens = JSON.parse(localStorage.getItem('adminFlashTokens') || '[]');
        
        if (!processedTokens.includes(token)) {
            console.log('Mostrando alerta flash en admin:', message);
            
            if (typeof Swal !== 'undefined') {
                Swal.fire({                                         
                    icon: 'success',
                    title: '¡Éxito!',
                    text: message,
                    confirmButtonColor: '#2f9f37',
                    confirmButtonText: '¡Perfecto!',
                    allowOutsideClick: true,
                    allowEscapeKey: true,
                    timer: 6000,
                    timerProgressBar: true,
                    position: 'center',
                    backdrop: true,
                    toast: false,
                    width: 'auto',
                    padding: '2em'
                });
            } else {
                alert(message);
            }
            
            // Marcar como procesado
            processedTokens.push(token);
            localStorage.setItem('adminFlashTokens', JSON.stringify(processedTokens));
        }
        
        // Limpiar meta tags
        flashMessage.remove();
        flashToken.remove();
    }
}

// Función para mostrar mensaje de bienvenida en admin
function showAdminWelcomeMessage() {
    const welcomeMessageMeta = document.querySelector('meta[name="welcome-message"]');
    
    if (welcomeMessageMeta) {
        const welcomeMessage = welcomeMessageMeta.getAttribute('content');
        console.log('Mostrando mensaje de bienvenida en admin:', welcomeMessage);
        
        // Determinar el tipo de mensaje para personalizar la alerta
        const isRegistrationMessage = welcomeMessage.includes('Tu cuenta ha sido creada exitosamente');
        const isAdminWelcome = welcomeMessage.includes('panel de administración');
        
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                icon: 'success',
                title: isRegistrationMessage ? '¡Bienvenido!' : (isAdminWelcome ? '¡Panel de Administración!' : '¡Hola de nuevo!'),
                text: welcomeMessage,
                confirmButtonText: 'Entendido',
                confirmButtonColor: '#2563eb',
                timer: isRegistrationMessage ? 10000 : (isAdminWelcome ? 8000 : 6000),
                timerProgressBar: true,
                allowOutsideClick: false,
                allowEscapeKey: false,
                showConfirmButton: true,
                showCloseButton: false,
                position: 'center',
                backdrop: true,
                focusConfirm: true,
                toast: false,
                width: 'auto',
                padding: '2em',
                customClass: {
                    popup: 'swal-welcome-success'
                },
                didOpen: () => {
                    // Animación del checkmark verde
                    const icon = document.querySelector('.swal2-success-circular-line-right');
                    if (icon) {
                        icon.style.animation = 'swal2-success-circular-line-right 0.75s ease-in-out';
                    }
                }
            }).then(() => {
                console.log('Mensaje de bienvenida de admin cerrado');
            });
        } else {
            console.warn('SweetAlert2 no está disponible, usando alert nativo');
            alert(welcomeMessage);
        }
        
        // Marcar como mostrado con timestamp
        sessionStorage.setItem('admin_welcome_shown', 'true');
        sessionStorage.setItem('last_admin_welcome_time', Date.now().toString());
        
        // Limpiar meta tag
        welcomeMessageMeta.remove();
        
        // Limpiar la sesión en el servidor
        fetch('/admin/clear-welcome-session', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
        }).catch(error => console.log('Error limpiando sesión de admin:', error));
    } else {
        console.log('No se encontró mensaje de bienvenida en admin');
    }
}

// Función para mostrar mensaje de bienvenida por defecto en admin (fallback)
function showDefaultAdminWelcomeMessage() {
    console.log('Mostrando mensaje de bienvenida por defecto en admin');
    
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            icon: 'success',
            title: '¡Panel de Administración!',
            text: '¡Bienvenido al panel de administración!',
            confirmButtonText: 'Entendido',
            confirmButtonColor: '#2563eb',
            timer: 8000,
            timerProgressBar: true,
            allowOutsideClick: false,
            allowEscapeKey: false,
            showConfirmButton: true,
            showCloseButton: false,
            position: 'center',
            backdrop: true,
            focusConfirm: true,
            toast: false,
            width: 'auto',
            padding: '2em',
            customClass: {
                popup: 'swal-welcome-success'
            },
            didOpen: () => {
                // Animación del checkmark verde
                const icon = document.querySelector('.swal2-success-circular-line-right');
                if (icon) {
                    icon.style.animation = 'swal2-success-circular-line-right 0.75s ease-in-out';
                }
            }
        }).then(() => {
            console.log('Mensaje de bienvenida por defecto de admin cerrado');
        });
    } else {
        console.warn('SweetAlert2 no está disponible, usando alert nativo');
        alert('¡Bienvenido al panel de administración!');
    }
}

// Función principal que se ejecuta cuando todo esté listo
function initializeAdminWelcomeSystem() {
    console.log('Inicializando sistema de bienvenida de admin...');
    
    // Prevenir ejecución múltiple
    if (adminWelcomeInitialized) {
        console.log('Sistema de admin ya inicializado, omitiendo...');
        return;
    }
    adminWelcomeInitialized = true;
    
    console.log('Sistema de admin inicializado correctamente');
    console.log('SweetAlert2 disponible:', typeof Swal !== 'undefined');
    
    // Esperar a que SweetAlert2 esté cargado
    setTimeout(() => {
        console.log('Ejecutando alertas de admin...');
        
        // Verificar si hay alertas flash primero
        const hasFlashAlert = document.querySelector('meta[name="flash-message"]');
        const hasWelcomeMessage = document.querySelector('meta[name="welcome-message"]');
        
        console.log('Meta tags encontrados en admin:');
        console.log('- flash-message:', hasFlashAlert ? hasFlashAlert.getAttribute('content') : 'NO ENCONTRADO');
        console.log('- welcome-message:', hasWelcomeMessage ? hasWelcomeMessage.getAttribute('content') : 'NO ENCONTRADO');
        
        if (hasWelcomeMessage) {
            // Priorizar mensaje de bienvenida (puede ser registro o login)
            const messageContent = hasWelcomeMessage.getAttribute('content');
            console.log('Mostrando mensaje de bienvenida en admin:', messageContent);
            
            // Determinar si es mensaje de registro, admin welcome o bienvenida normal
            if (messageContent.includes('Tu cuenta ha sido creada exitosamente')) {
                console.log('Es un mensaje de registro - mostrando inmediatamente');
                setTimeout(() => {
                    showAdminWelcomeMessage();
                }, 300); // 0.3 segundos para registro
            } else if (messageContent.includes('panel de administración')) {
                console.log('Es un mensaje de bienvenida de admin - mostrando en 0.5s');
                setTimeout(() => {
                    showAdminWelcomeMessage();
                }, 500); // 0.5 segundos para bienvenida de admin
            } else {
                console.log('Es un mensaje de bienvenida normal - mostrando en 0.5s');
                setTimeout(() => {
                    showAdminWelcomeMessage();
                }, 500); // 0.5 segundos para bienvenida normal
            }
        } else if (hasFlashAlert) {
            // Si hay alerta flash (fallback), mostrarla inmediatamente
            console.log('Mostrando alerta flash en admin...');
            showAdminFlashAlert();
        } else {
            // Verificar si ya se mostró la alerta en esta sesión
            const adminWelcomeShown = sessionStorage.getItem('admin_welcome_shown');
            if (!adminWelcomeShown) {
                console.log('No se encontraron meta tags, mostrando alerta de bienvenida por defecto en admin');
                showDefaultAdminWelcomeMessage();
                // Marcar como mostrado en esta sesión con timestamp
                sessionStorage.setItem('admin_welcome_shown', 'true');
                sessionStorage.setItem('last_admin_welcome_time', Date.now().toString());
            } else {
                console.log('Alerta de bienvenida de admin ya mostrada en esta sesión, omitiendo');
            }
        }
    }, 1000); // 1 segundo para asegurar que todo esté cargado
}

// Ejecutar cuando el DOM esté listo
document.addEventListener('DOMContentLoaded', () => {
    // Verificar si hay sesión activa (usuario autenticado)
    const hasAuthUser = document.querySelector('meta[name="csrf-token"]');
    if (!hasAuthUser) {
        console.log('No hay sesión activa, limpiando sessionStorage de admin');
        sessionStorage.removeItem('welcome_shown');
        sessionStorage.removeItem('admin_welcome_shown');
    } else {
        // Si hay sesión activa, verificar si es una nueva sesión
        // Comparar con el timestamp de la última vez que se mostró la alerta
        const lastWelcomeTime = sessionStorage.getItem('last_admin_welcome_time');
        const currentTime = Date.now();
        const sessionTimeout = 30 * 60 * 1000; // 30 minutos
        
        // Si ha pasado más de 30 minutos o es la primera vez, limpiar y mostrar
        if (!lastWelcomeTime || (currentTime - parseInt(lastWelcomeTime)) > sessionTimeout) {
            console.log('Nueva sesión admin detectada o timeout alcanzado, limpiando sessionStorage');
            sessionStorage.removeItem('welcome_shown');
            sessionStorage.removeItem('admin_welcome_shown');
        }
    }
    
    initializeAdminWelcomeSystem();
});

// También ejecutar cuando la página termine de cargar (solo como respaldo)
window.addEventListener('load', () => {
    if (!adminWelcomeInitialized) {
        setTimeout(() => {
            initializeAdminWelcomeSystem();
        }, 1000);
    }
});
