...
<div id="{{ $id }}" 
     class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] transition-opacity duration-300 {{ $show ? 'opacity-100' : 'opacity-0 pointer-events-none' }}"
    ...
    <div class="bg-white rounded-lg p-8 shadow-2xl max-w-sm w-full mx-4 text-center">
        ...
        <!-- Texto de carga -->
        <h3 class="loading-text text-lg font-semibold text-gray-800">{{ $text }}</h3>
    </div>
</div>
