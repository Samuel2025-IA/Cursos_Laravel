...
@section('title', 'Editar Curso - Diócesis de Apartadó')
...
<div class="space-y-6">
    <!-- Encabezado -->
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Editar Curso</h1>
        <p class="text-gray-600">Modifica la información y el formulario del curso</p>
    </div>
    ...
    <form id="course-form" method="POST" action="{{ route('cursos.update', $curso) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="mt-6">
                <label class="flex items-center">
                    <input type="checkbox" name="activo" value="1" 
                           ...
                           class="rounded border-gray-300 text-[#2f9f37] focus:ring-[#2f9f37]">
                    <span class="ml-2 text-sm font-medium text-gray-700">Curso activo</span>
                </label>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mr-0 lg:mr-24">
            ...
            <div class="px-6 py-8 border-b border-gray-200">
                <input type="text" 
                       ...
                       value="{{ old('form_title', $curso->form_title ?? '') }}">
                <input type="text" 
                       id="form-description" 
                       ...
                       value="{{ old('form_description', $curso->form_description ?? '') }}">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        courseForm.addEventListener('submit', function(e) {
            ...
            if (formFields.length === 0) {
                ...
                Swal.fire({
                    ...
                    text: 'Debes agregar al menos un campo de pregunta para actualizar el curso.',
                    confirmButtonColor: '#dc2626',
                    confirmButtonText: 'Entiendo'
                });
                return false;
    ...
    // Restaurar borrador o cargar campos existentes del curso
    const draft = localStorage.getItem(DRAFT_KEY);
    const existingFields = @json($curso->form_fields ?? []);
    
    if (draft && draft.trim() !== '') {
        // Si hay un borrador, usarlo
    ...
    } else if (existingFields && existingFields.length > 0) {
        // Si no hay borrador pero hay campos existentes, cargarlos
        const container = document.getElementById('form-fields-container');
        existingFields.forEach((field, index) => {
            ...
            // Llenar los datos del campo
            const fieldElement = container.querySelector(`[data-field-type="${field.type}"]:last-child`);
            if (fieldElement) {
                // Llenar pregunta
                const questionInput = fieldElement.querySelector('input[name*="[question]"]');
                if (questionInput && field.question) {
                    questionInput.value = field.question;
                }
                
                // Llenar required
                const requiredCheckbox = fieldElement.querySelector('input[name*="[required]"]');
                if (requiredCheckbox) {
                ...
                if (field.options && field.options.length > 0) {
                    ...
                    if (optionsContainer) {
                        // Eliminar opciones por defecto y crear las del curso
                        optionsContainer.innerHTML = '';
                        field.options.forEach((option, optIndex) => {
                            const iconSVG = field.type === 'multiple-choice' 
                ...
                // Para imágenes y videos, cargar las rutas existentes
                if (field.type === 'image' && field.image_path) {
                    const imagePreview = fieldElement.querySelector(`#image_preview_${fieldCounter}`);
                    const imageImg = fieldElement.querySelector(`#image_preview_img_${fieldCounter}`);
                    const widthInput = fieldElement.querySelector(`#image_width_${fieldCounter}`);
                    const heightInput = fieldElement.querySelector(`#image_height_${fieldCounter}`);
                    
                    if (imagePreview && imageImg) {
                        imageImg.src = '/storage/' + field.image_path;
                        imagePreview.classList.remove('hidden');
                        if (widthInput) widthInput.value = field.image_width || '100%';
                        if (heightInput) heightInput.value = field.image_height || 'auto';
                    }
                }
                
                if (field.type === 'video') {
                    const videoPreview = fieldElement.querySelector(`#video_preview_${fieldCounter}`);
                    const videoElement = fieldElement.querySelector(`#video_preview_element_${fieldCounter}`);
                    const videoUrlInput = fieldElement.querySelector(`#video_url_${fieldCounter}`);
                    const widthInput = fieldElement.querySelector(`#video_width_${fieldCounter}`);
                    const heightInput = fieldElement.querySelector(`#video_height_${fieldCounter}`);
                    
                    if (field.video_url && videoUrlInput) {
                        videoUrlInput.value = field.video_url;
                    } else if (field.video_path && videoPreview && videoElement) {
                        videoElement.src = '/storage/' + field.video_path;
                        videoPreview.classList.remove('hidden');
                    }
                    if (widthInput) widthInput.value = field.video_width || '100%';
                    if (heightInput) heightInput.value = field.video_height || 'auto';
                }
            }
        });
    }
